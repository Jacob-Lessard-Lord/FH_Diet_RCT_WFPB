...
lmer_reg_strat <- function(data_df, var_depend, stratification, formule) {
  ...
  modele_1 <- lmer(as.formula(formule),
                   data = data_df_1,
                   REML = TRUE)
  
  # Entraînement du modèle selon la 2e référence
  modele_2 <- lmer(as.formula(formule),
                   data = data_df_2,
                   REML = TRUE)
  ...
  results_reg_1 <- data.frame(Outcome = var_depend,
                              ...
                              Strat_level = level_1,
                              WFPB_SAD_mean = modele_1_summary$coefficients[rownames(modele_1_summary$coefficients) == "txWFPB", "Estimate"],
                              WFPB_SAD_lower = modele_1_confint[rownames(modele_1_confint) == "txWFPB", 1],
                              WFPB_SAD_upper = modele_1_confint[rownames(modele_1_confint) == "txWFPB", 2],
                              p_value = modele_1_summary$coefficients[rownames(modele_1_summary$coefficients) == "txWFPB", "Pr(>|t|)"],
  ...
  results_reg_2 <- data.frame(Outcome = var_depend,
                              Stratification = stratification,
                              Strat_level = level_2,
                              WFPB_SAD_mean = modele_2_summary$coefficients[rownames(modele_2_summary$coefficients) == "txWFPB", "Estimate"],
                              WFPB_SAD_lower = modele_2_confint[rownames(modele_2_confint) == "txWFPB", 1],
                              WFPB_SAD_upper = modele_2_confint[rownames(modele_2_confint) == "txWFPB", 2],
                              p_value = modele_2_summary$coefficients[rownames(modele_2_summary$coefficients) == "txWFPB", "Pr(>|t|)"],
                              p_value_interaction = modele_2_summary$coefficients[nrow(modele_2_summary$coefficients), "Pr(>|t|)"])
  
  # Calcul de la moyenne post-SAD pour calculer le % de changement
  moyenne_SAD <- subset(data_df, subset = tx == "SAD") %>% group_by(eval(parse(text = stratification))) %>% get_summary_stats(all_of(var_depend), type = "mean")
  
  # Mise en forme du mean (95% CI) absolu et relatif
  results_reg_1 <- results_reg_1 %>%
    mutate(n = as.numeric(moyenne_SAD[moyenne_SAD[, 1] == level_1, "n"]),
           WFPB_SAD_mean_ci_abs = paste0(round(WFPB_SAD_mean, digits = 2), " (", round(WFPB_SAD_lower, digits = 2), ", ", round(WFPB_SAD_upper, digits = 2), ")"),
           WFPB_SAD_mean_ci_rel = paste0(round((WFPB_SAD_mean / moyenne_SAD[moyenne_SAD[, 1] == level_1, "mean"]) * 100, digits = 1),
                                        "% (", round((WFPB_SAD_lower / moyenne_SAD[moyenne_SAD[, 1] == level_1, "mean"]) * 100, digits = 1), "%, ",
                                        round((WFPB_SAD_upper / moyenne_SAD[moyenne_SAD[, 1] == level_1, "mean"]) * 100, digits = 1), "%)"))
  
  results_reg_2 <- results_reg_2 %>%
    mutate(n = as.numeric(moyenne_SAD[moyenne_SAD[, 1] == level_2, "n"]),
           WFPB_SAD_mean_ci_abs = paste0(round(WFPB_SAD_mean, digits = 2), " (", round(WFPB_SAD_lower, digits = 2), ", ", round(WFPB_SAD_upper, digits = 2), ")"),
           WFPB_SAD_mean_ci_rel = paste0(round((WFPB_SAD_mean / moyenne_SAD[moyenne_SAD[, 1] == level_2, "mean"]) * 100, digits = 1),
                                        "% (", round((WFPB_SAD_lower / moyenne_SAD[moyenne_SAD[, 1] == level_2, "mean"]) * 100, digits = 1), "%, ",
                                        round((WFPB_SAD_upper / moyenne_SAD[moyenne_SAD[, 1] == level_2, "mean"]) * 100, digits = 1), "%)"))
