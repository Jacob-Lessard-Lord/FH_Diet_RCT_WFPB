...
lmer_reg_strat <- function(data_df, var_depend, stratification, formule) {
  ...
  # Calcul de la moyenne apres 7 jours pour calculer le % de changement
  moyenne_d7 <- subset(data_df, subset = vas_day_factor == "After 7 days") %>%
    group_by(eval(parse(text = stratification))) %>% get_summary_stats(var_depend, type = "mean")
  
  # Mise en forme du mean (95% CI) absolu et relatif
  results_reg_1 <- results_reg_1 %>%
    mutate(n = as.numeric(moyenne_d7[moyenne_d7[, 1] == level_1, "n"]),
           d28_d7_mean_ci_abs = paste0(round(d28_d7_mean, digits = 2), " (", round(d28_d7_lower, digits = 2), ", ", round(d28_d7_upper, digits = 2), ")"),
           d28_d7_mean_ci_rel = paste0(round((d28_d7_mean / moyenne_d7[moyenne_d7[, 1] == level_1, "mean"]) * 100, digits = 1),
                                       "% (", round((d28_d7_lower / moyenne_d7[moyenne_d7[, 1] == level_1, "mean"]) * 100, digits = 1), "%, ",
                                       round((d28_d7_upper / moyenne_d7[moyenne_d7[, 1] == level_1, "mean"]) * 100, digits = 1), "%)"))
  
  results_reg_2 <- results_reg_2 %>%
    mutate(n = as.numeric(moyenne_d7[moyenne_d7[, 1] == level_2, "n"]),
           d28_d7_mean_ci_abs = paste0(round(d28_d7_mean, digits = 2), " (", round(d28_d7_lower, digits = 2), ", ", round(d28_d7_upper, digits = 2), ")"),
           d28_d7_mean_ci_rel = paste0(round((d28_d7_mean / moyenne_d7[moyenne_d7[, 1] == level_2, "mean"]) * 100, digits = 1),
                                       "% (", round((d28_d7_lower / moyenne_d7[moyenne_d7[, 1] == level_2, "mean"]) * 100, digits = 1), "%, ",
                                       round((d28_d7_upper / moyenne_d7[moyenne_d7[, 1] == level_2, "mean"]) * 100, digits = 1), "%)"))
