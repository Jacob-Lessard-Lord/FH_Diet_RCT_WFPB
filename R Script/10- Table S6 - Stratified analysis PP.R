...
model_ldlc_age <- lmer(ldlc ~ tx*age + tx + seq + bmi + weight_delta + (1|nomat), data = data_post_pp, REML = TRUE)
...
# Test de l'interaction en continu # 
model_apob_neph_age <- lmer(apob_neph ~ tx*age + tx + seq + bmi + weight_delta + (1|nomat), data = data_post_pp, REML = TRUE)
