...
model_ldlc_age <- lmer(ldlc ~ tx*age + tx + age + sex + hfgenotype + seq + bmi + weight_delta + (1|nomat), data = data_post_itt, REML = TRUE)
...
# Test de l'interaction en continu # 
model_apob_neph_age <- lmer(apob_neph ~ tx*age + tx + age + sex + hfgenotype + seq + bmi + weight_delta + (1|nomat), data = data_post_itt, REML = TRUE)
...
write.xlsx(cbind(results_reg_ldlc, results_reg_apob_neph), file = "Table S6.xlsx")