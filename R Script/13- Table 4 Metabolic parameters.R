...
influence_glp1 <- influence(lmer(as.formula(paste("glp1", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE)
cook_glp1 <- data.frame(Num_obs = 1:length(cooks.distance(influence_glp1)),
                        Cook_dist = cooks.distance(influence_glp1))
cook_glp1[order(cook_glp1$Cook_dist, decreasing = TRUE), ]
data_post_pp[c(39:40), c("nomat", "Visites", "tx", "glp1")]

# Modèle + résultats du modèle
model_glp1 <- lmer_reg(subset(data_post_pp, nomat != "021"), "glp1", formule_default)
...
# Pour l'article on conserve les résultats avec le sujet extrême
model_glp1 <- lmer_reg(data_post_pp, "glp1")
...
write.xlsx(results_reg, file = "Table 4.xlsx")
