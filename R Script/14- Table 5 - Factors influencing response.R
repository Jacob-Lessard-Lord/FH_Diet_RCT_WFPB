...
all_model_ldlc_rel <- ols_step_all_possible(modele_complet_ldlc_rel)

# R2 ajusté
head(as.data.frame(all_model_ldlc_rel$result[order(-all_model_ldlc_rel$result$adjr), ]), 5)

# Modele reduit
modele_reduit_ldlc_rel <- lm(as.formula(paste("ldlc ~", gsub(x = all_model_ldlc_rel$result[order(-all_model_ldlc_rel$result$adjr), "predictors"][1],
                                                             pattern = " ", replacement = " + "))),
                             data = data_diff_pp)

# Multicolinearite - les doivent etre < 10 
ols_vif_tol(modele_reduit_ldlc_rel)

# Vérification des hypothèses du modèle
plot(x = modele_reduit_ldlc_rel$fitted.values, y = modele_reduit_ldlc_rel$residuals) # Homogénéité # Pas la meilleure homogénéité ...
qqnorm(resid(modele_reduit_ldlc_rel)) # Normalité des résidus Q-Q Plot (point)
qqline(resid(modele_reduit_ldlc_rel)) # Normalité des résidus Q-Q Plot (ligne)
shapiro_test(resid(modele_reduit_ldlc_rel)) # Normalité des résidus (Shapiro test)

# Influence des observations
influencePlot(modele_reduit_ldlc_rel)
influenceIndexPlot(modele_reduit_ldlc_rel)

# Résultats du modèle complet
summary(modele_reduit_ldlc_rel)

# Extraction des résultats
results_reg_reduit_ldlc_rel <- merge(x = rownames_to_column(as.data.frame(summary(modele_reduit_ldlc_rel)$coefficients), var = "Variable"),
                                     y = rownames_to_column(as.data.frame(confint(modele_reduit_ldlc_rel)), var = "Variable"),
                                     by = "Variable", sort = FALSE)[-1, ] # Retrait de l'intercept

# Renommer les colonnes
colnames(results_reg_reduit_ldlc_rel) <- c("Variable", "Estimate", "Std_error", "t_value", "p_value", "lower_ci", "upper_ci")

# Intégrer les r2 partiels aux résultats
results_reg_reduit_ldlc_rel <- cbind(subset(results_reg_reduit_ldlc_rel, subset = Variable != "hfgenotypeOther"),
                                     data.frame(Variable2 = rsq.partial(modele_reduit_ldlc_rel)$variable,
                                                partial_r2 = rsq.partial(modele_reduit_ldlc_rel)$partial.rsq))

# WARNINGS NORMAUX #

# Mise en forme des résultats
results_reg_reduit_ldlc_rel <- results_reg_reduit_ldlc_rel %>%
  mutate(Estimate = round(Estimate, digits = 2),
         lower_ci = round(lower_ci, digits = 2),
         upper_ci = round(upper_ci, digits = 2),
         beta_ci = paste0(Estimate, " (", lower_ci, ", ", upper_ci, ")"),
...
# Sélection des résultats pertinents
results_reg_reduit_ldlc_rel <- results_reg_reduit_ldlc_rel %>% select(c("Variable", "beta_ci", "partial_r2", "p_value"))

# Ajout du r2 du modèle
results_reg_reduit_ldlc_rel <- rbind(
  results_reg_reduit_ldlc_rel,
  data.frame(Variable = "Model",
             beta_ci = NA,
             partial_r2 = paste0(round(ols_regress(modele_reduit_ldlc_rel)$rsq * 100, 1), " (", round(ols_regress(modele_reduit_ldlc_rel)$adjr * 100, 1), ")"),
             p_value = round(ols_regress(modele_reduit_ldlc_rel)$p, 4))
)
...
# Test de tous les sous-modèles possibles
all_model_apob_rel <- ols_step_all_possible(modele_complet_apob_rel)

# R2 ajusté
head(as.data.frame(all_model_apob_rel$result[order(-all_model_apob_rel$result$adjr), ]), 5)

# PRESS
head(as.data.frame(all_model_apob_rel$result[order(all_model_apob_rel$result$fpe), ]), 5)

# Modele reduit
modele_reduit_apob_rel <- lm(as.formula(paste("apob_neph ~", gsub(x = all_model_apob_rel$result[order(-all_model_apob_rel$result$adjr), "predictors"][1],
                                                             pattern = " ", replacement = " + "))),
                             data = data_diff_pp)

# Multicolinearite - les doivent etre < 10 
ols_vif_tol(modele_reduit_apob_rel)

# Vérification des hypothèses du modèle
plot(x = modele_reduit_apob_rel$fitted.values, y = modele_reduit_apob_rel$residuals) # Homogénéité
qqnorm(resid(modele_reduit_apob_rel)) # Normalité des résidus Q-Q Plot (point)
qqline(resid(modele_reduit_apob_rel)) # Normalité des résidus Q-Q Plot (ligne)
shapiro_test(resid(modele_reduit_apob_rel)) # Normalité des résidus (Shapiro test)

# Influence des observations
influencePlot(modele_reduit_apob_rel)
influenceIndexPlot(modele_reduit_apob_rel)

# Résultats du modèle complet
summary(modele_reduit_apob_rel)

# Extraction des résultats
results_reg_reduit_apob_rel <- merge(x = rownames_to_column(as.data.frame(summary(modele_reduit_apob_rel)$coefficients), var = "Variable"),
                                     y = rownames_to_column(as.data.frame(confint(modele_reduit_apob_rel)), var = "Variable"),
                                     by = "Variable", sort = FALSE)[-1, ] # Retrait de l'intercept

# Renommer les colonnes
colnames(results_reg_reduit_apob_rel) <- c("Variable", "Estimate", "Std_error", "t_value", "p_value", "lower_ci", "upper_ci")

# Intégrer les r2 partiels aux résultats
results_reg_reduit_apob_rel <- cbind(results_reg_reduit_apob_rel,
                                     data.frame(Variable2 = rsq.partial(modele_reduit_apob_rel)$variable,
                                                partial_r2 = rsq.partial(modele_reduit_apob_rel)$partial.rsq))
# WARNINGS NORMAUX #

# Mise en forme des résultats
results_reg_reduit_apob_rel <- results_reg_reduit_apob_rel %>%
  mutate(Estimate = round(Estimate, digits = 2),
         lower_ci = round(lower_ci, digits = 2),
         upper_ci = round(upper_ci, digits = 2),
         beta_ci = paste0(Estimate, " (", lower_ci, ", ", upper_ci, ")"),
...
# Sélection des résultats pertinents
results_reg_reduit_apob_rel <- results_reg_reduit_apob_rel %>% select(c("Variable", "beta_ci", "partial_r2", "p_value"))

# Ajout du r2 du modèle
results_reg_reduit_apob_rel <- rbind(
  results_reg_reduit_apob_rel,
  data.frame(Variable = "Model",
             beta_ci = NA,
             partial_r2 = paste0(round(ols_regress(modele_reduit_apob_rel)$rsq * 100, 1), " (", round(ols_regress(modele_reduit_apob_rel)$adjr * 100, 1), ")"),
             p_value = round(ols_regress(modele_reduit_apob_rel)$p, 4))
)
...
## Ordre des variables
variable_order_ldlc <- results_reg_complet_ldlc_rel$Variable

## Relatif
results_ldlc_rel <- merge(x = results_reg_complet_ldlc_rel, y = results_reg_reduit_ldlc_rel, by = "Variable", all = TRUE, sort = FALSE)
results_ldlc_rel <- results_ldlc_rel[match(variable_order_ldlc, results_ldlc_rel$Variable), ]

# ApoB

## Ordre des variables
variable_order_apob <- results_reg_complet_apob_rel$Variable

## Relatif
results_apob_rel <- merge(x = results_reg_complet_apob_rel, y = results_reg_reduit_apob_rel, by = "Variable", all = TRUE, sort = FALSE)
results_apob_rel <- results_apob_rel[match(variable_order_apob, results_apob_rel$Variable), ]

# Combinaison des resultats
results_relative <- rbind(results_ldlc_rel, results_apob_rel)

# Exportation des résultats
write.xlsx(results_relative, file = "Table 5.xlsx")
