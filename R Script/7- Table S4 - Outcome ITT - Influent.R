...
influence_hdlc <- influence(lmer(as.formula(paste("hdlc", "~", formule_default)), data = data_post_itt, REML = TRUE), obs = TRUE)
cook_hdlc <- data.frame(Num_obs = 1:length(cooks.distance(influence_hdlc)),
                        Cook_dist = cooks.distance(influence_hdlc))
cook_hdlc[order(cook_hdlc$Cook_dist, decreasing = TRUE), ] # Ligne 41 et 42 sont les plus influente
data_post_itt[41:42, "nomat"] # Sujet 022 est influent

model_hdlc <- lmer_reg(subset(data_post_itt, nomat != "022"), "hdlc") # Retrait du sujet 022
...
# Le problème s'est vraiment amélioré avec le retrait de cette observation !
...
# Extraction des distances de Cook
influence_tg <- influence(lmer(as.formula(paste("tg", "~", formule_default)), data = data_post_itt, REML = TRUE), obs = TRUE)
cook_tg <- data.frame(Num_obs = 1:length(cooks.distance(influence_tg)),
                      Cook_dist = cooks.distance(influence_tg))
cook_tg[order(cook_tg$Cook_dist, decreasing = TRUE), ] # Ligne 3 et 4 sont les plus influente
data_post_itt[c(3:4), "nomat"] # Sujet 002 est influent

model_tg <- lmer_reg(subset(data_post_itt, nomat != "002"), "tg", log_transform = TRUE) # Retrait du sujet 002
...
# Modèle + résultats du modèle
model_lpa_neph <- lmer_reg(data_post_itt, "lpa_neph")
...
# 2 données semblent très influentes + nuire à l'homogénéité de la variance et la normalité des résidus

# Extraction des distances de Cook
influence_lpa_neph <- influence(lmer(as.formula(paste("lpa_neph", "~", formule_default)), data = data_post_itt, REML = TRUE), obs = TRUE)
...
data_post_itt[c(25:26, 59:60), "nomat"] # Retrait du sujet 031, car ce n'est pas le sujet 014 qui nuit le plus aux postulats

model_lpa_neph <- lmer_reg(subset(data_post_itt, nomat != "031"), "lpa_neph", digits_results = 1) # Retrait du sujet 031
...
# Le problème s'est vraiment amélioré avec le retrait de cette observation !
# Ce n'est pas parfait, mais rien d'autre ne fonctionne et ce n'est pas significatif de toute facon
...
# Extraction des distances de Cook
influence_dbp <- influence(lmer(as.formula(paste("dbp", "~", formule_default)), data = data_post_itt, REML = TRUE), obs = TRUE)
...
data_post_itt[c(9:10), "nomat"]

model_dbp <- lmer_reg(subset(data_post_itt, nomat != "005"), "dbp", digits_results = 1) # Retrait du sujet 005
...
write.xlsx(results_reg, file = "Table S4.xlsx")