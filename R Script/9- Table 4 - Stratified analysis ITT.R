...
#### Sex ####
model_ldlc_sex <- lmer_reg_strat(data_post_itt, "ldlc", "sex", 
                                 "ldlc ~ tx*sex + tx  + seq + bmi + weight_delta + (1|nomat)")

# Validation des hypothèses du modèle
plot(influence(lmer(ldlc ~ tx*sex + tx + seq + bmi + weight_delta + (1|nomat),
...
#### LDLR ####
model_ldlc_hfgenotype <- lmer_reg_strat(subset(data_post_itt, subset = hfgenotype != "Other") %>% mutate(hfgenotype = droplevels(hfgenotype)),
                                        "ldlc", "hfgenotype",
                                        "ldlc ~ tx*hfgenotype +  tx + seq + bmi + weight_delta + (1|nomat)")

# Validation des hypothèses du modèle
plot(influence(lmer(ldlc ~ tx*hfgenotype +  tx + seq + bmi + weight_delta + (1|nomat),
                    data = data_post_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
model_ldlc_age_cat_40 <- lmer_reg_strat(data_post_itt, "ldlc", "age_cat_40",
...
model_ldlc_age <- lmer(ldlc ~ tx*age + tx + seq + bmi + weight_delta + (1|nomat), data = data_post_itt, REML = TRUE)
...
#### Sex ####
model_apob_neph_sex <- lmer_reg_strat(data_post_itt, "apob_neph", "sex", 
                                      "apob_neph ~ tx*sex +  tx + seq + bmi + weight_delta + (1|nomat)")

# Validation des hypothèses du modèle
plot(influence(lmer(apob_neph ~ tx*sex +  tx + seq + bmi + weight_delta + (1|nomat),
                    data = data_post_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
#### LDLR ####
model_apob_neph_hfgenotype <- lmer_reg_strat(subset(data_post_itt, subset = hfgenotype != "Other") %>% mutate(hfgenotype = droplevels(hfgenotype)),
                                        "apob_neph", "hfgenotype",
                                        "apob_neph ~ tx*hfgenotype +  tx + seq + bmi + weight_delta + (1|nomat)")

# Validation des hypothèses du modèle
plot(influence(lmer(apob_neph ~ tx*hfgenotype + tx + seq + bmi + weight_delta + (1|nomat),
                    data = data_post_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
#### Age stratifié à 40 ans ####
model_apob_neph_age_cat_40 <- lmer_reg_strat(data_post_itt, "apob_neph", "age_cat_40",
...
# Test de l'interaction en continu # 
model_apob_neph_age <- lmer(apob_neph ~ tx*age + tx + seq + bmi + weight_delta + (1|nomat), data = data_post_itt, REML = TRUE)
...
write.xlsx(cbind(results_reg_ldlc, results_reg_apob_neph), file = "Table 4.xlsx")
