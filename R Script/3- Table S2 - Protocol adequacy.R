...
model_energy <- lmer_reg(data_post_itt, "kcal_mean", "tx + age + sex + seq + bmi + (1|nomat)", digits_results = 0)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("kcal_mean", "~", "tx + age + sex + seq + bmi + (1|nomat)")),
...
# Modèle + résultats du modèle
model_compliance <- lmer_reg(data_post_itt, "comp_mean", "tx + age + sex + seq + (1|nomat)", digits_results = 1)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("comp_mean", "~", "tx + age + sex + seq + (1|nomat)")),
                    data = data_post_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Les postulats ne sont pas vraiment respecté, mais avec une aussi bonne compliance, ça sera impossible de les respecter et on montre simplement que la compliance
# est équivalent dans les 2 diètes

#### Weight post intervention ####

# Modèle + résultats du modèle
model_weight_post <- lmer_reg(data_post_itt, "weight", "tx + age + sex + seq + (1|nomat)", digits_results = 1)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("weight", "~", "tx + age + sex + seq + (1|nomat)")),
                    data = data_post_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_weight_change <- lmer_reg(data_post_itt, "weight_delta", "tx + age + sex + seq + bmi + (1|nomat)")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("weight_delta", "~", "tx + age + sex + seq + bmi + (1|nomat)")),
                    data = data_post_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_weight_sad <- lmer(SAD ~ Visites + age + sex + seq + (1|nomat), data = data_weight_itt, REML = TRUE)
summary(model_weight_sad)

# Validation des hypothèses du modèle
plot(influence(lmer(SAD ~ Visites + age + sex + seq + (1|nomat), data = data_weight_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_weight_wfpb <- lmer(WFPB ~ Visites + age + sex + seq + (1|nomat), data = data_weight_itt, REML = TRUE)
summary(model_weight_wfpb)

# Validation des hypothèses du modèle
plot(influence(lmer(WFPB ~ Visites + age + sex + seq + (1|nomat), data = data_weight_itt, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
