...
# Modèle + résultats du modèle
model_ten_year_risk_ascvd <- lmer_reg(data_post_pp, "ten_year_risk_ascvd", "tx + seq + bmi + weight_delta + (1|nomat)", digits_results = 1)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("ten_year_risk_ascvd", "~", "tx + seq + bmi + weight_delta + (1|nomat)")), data = data_post_pp, REML = TRUE),
...
# Problème de normalité des résidus !!! Transformation log
model_ten_year_risk_ascvd <- lmer_reg(data_post_pp, "ten_year_risk_ascvd", "tx + seq + bmi + weight_delta + (1|nomat)",
                                      log_transform = TRUE) # Transformation log

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("log(ten_year_risk_ascvd)", "~", "tx + seq + bmi + weight_delta + (1|nomat)")), data = data_post_pp, REML = TRUE),
               obs = TRUE), which = "cook") # Distance de Cook
plot(model_ten_year_risk_ascvd$modele) # Homogénéité de la variance
qqnorm(resid(model_ten_year_risk_ascvd$modele)) # Normalité des résidus Q-Q Plot (point)
qqline(resid(model_ten_year_risk_ascvd$modele)) # Normalité des résidus Q-Q Plot (ligne)
