...
formule_default <- "tx + age + sex + hfgenotype + seq + bmi + weight_delta + (1|nomat)"
...
model_ldlc <- lmer_reg(data_post_pp, "ldlc")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("ldlc", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_ldlc_lpa_corr <- lmer_reg(data_post_pp, "ldlc_lpa_corr")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("ldlc_lpa_corr", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_chol <- lmer_reg(data_post_pp, "chol")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("chol", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_nhdlc <- lmer_reg(data_post_pp, "nhdlc")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("nhdlc", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_apoa1_neph <- lmer_reg(data_post_pp, "apoa1_neph")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("apoa1_neph", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_apob_neph <- lmer_reg(data_post_pp, "apob_neph")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("apob_neph", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_crp_neph <- lmer_reg(data_post_pp, "crp_neph")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("crp_neph", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Problème de normalité des résidus !!! Transformation log
model_crp_neph <- lmer_reg(data_post_pp, "crp_neph", log_transform = TRUE) # Transformation log

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("log(crp_neph)", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
plot(model_crp_neph$modele) # Homogénéité de la variance
qqnorm(resid(model_crp_neph$modele)) # Normalité des résidus Q-Q Plot (point)
qqline(resid(model_crp_neph$modele)) # Normalité des résidus Q-Q Plot (ligne)
shapiro_test(resid(model_crp_neph$modele)) # Normalité des résidus (Shapiro test)
...
# Modèle + résultats du modèle
model_glufast <- lmer_reg(data_post_pp, "glufast")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("glufast", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_ins2018 <- lmer_reg(data_post_pp, "ins2018", digits_results = 1)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("ins2018", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_hba1c <- lmer_reg(data_post_pp, "hba1c")

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("hba1c", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_sbp <- lmer_reg(data_post_pp, "sbp", digits_results = 1)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("sbp", "~", formule_default)), data = data_post_pp, REML = TRUE), obs = TRUE), which = "cook") # Distance de Cook
...
# Modèle + résultats du modèle
model_ten_year_risk_ascvd <- lmer_reg(data_post_pp, "ten_year_risk_ascvd", "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)", digits_results = 1)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("ten_year_risk_ascvd", "~", "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)")), data = data_post_pp, REML = TRUE),
...
# Problème de normalité des résidus !!! Transformation log
model_ten_year_risk_ascvd <- lmer_reg(data_post_pp, "ten_year_risk_ascvd", "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)",
                                      log_transform = TRUE) # Transformation log

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("log(ten_year_risk_ascvd)", "~", "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)")), data = data_post_pp, REML = TRUE),
               obs = TRUE), which = "cook") # Distance de Cook
plot(model_ten_year_risk_ascvd$modele) # Homogénéité de la variance
qqnorm(resid(model_ten_year_risk_ascvd$modele)) # Normalité des résidus Q-Q Plot (point)
qqline(resid(model_ten_year_risk_ascvd$modele)) # Normalité des résidus Q-Q Plot (ligne)
...
write.xlsx(results_reg, file = "Table S3.xlsx")