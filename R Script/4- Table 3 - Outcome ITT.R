...
# Modèle + résultats du modèle
model_ten_year_risk_ascvd <- lmer_reg(data_post_itt, "ten_year_risk_ascvd", "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)", digits_results = 1)

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("ten_year_risk_ascvd", "~", "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)")), data = data_post_itt, REML = TRUE), obs = TRUE),
...
# Problème de normalité des résidus !!! Transformation log
model_ten_year_risk_ascvd <- lmer_reg(data_post_itt, "ten_year_risk_ascvd", formule = "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)",
                                      log_transform = TRUE) # Transformation log

# Validation des hypothèses du modèle
plot(influence(lmer(as.formula(paste("log(ten_year_risk_ascvd)", "~", "tx + hfgenotype + seq + bmi + weight_delta + (1|nomat)")), data = data_post_itt, REML = TRUE),
